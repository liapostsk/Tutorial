...
import com.ccsw.TutorialEntregable.author.model.Author;
...
public class AuthorController {
    ...
    @Autowired
    AuthorService authorService; //Servicio de la logica de los autores

    @Autowired
    ModelMapper mapper; //herramienta de traduccion entre Entity y Dtos
    ...
    @RequestMapping(path = "", method = RequestMethod.POST) //Usamos POST porque es para tipos complejos, GET se usaria si fuese algo simple
    public Page<AuthorDto> findPage(@RequestBody AuthorSearchDto dto) { //@RequestBody: datos del body de solicitud HTTP de pasan a tipo AuthorSearchDto

        Page<Author> page = this.authorService.findPage(dto);
        //devuelve datos en AuthorDto, mapper para la traduccion de Author a AuthorDto
